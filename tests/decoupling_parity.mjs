 ...
 *
 * Reads {"doc": <stream document>, "intervals": [[start, end], ...]} on stdin and writes
 * what web/decoupling.js makes of it to stdout, so the Python test can compare the two
 * implementations number by number. NaN has no JSON spelling, so it travels as null and
...
const data = prepare(loadStreams(job.doc), { doubleCadence: job.doubleCadence });
