 ...
 * strava_data/streams.py). The GitHub Action publishes each activity's sample streams as
 * JSON; everything below turns those into the numbers the page shows, so there is no
 * server doing analysis and no API call when you drag a slider.
 *
...
/**
 * A published stream document -> arrays in SI units.
 *
 * Series Strava never supplied come back all-NaN (all-false for `moving`) at the right
